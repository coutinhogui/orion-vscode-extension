export interface ResourceIntent {
  readonly command: 'setup' | 'review' | 'pipeline' | 'api' | 'blazor' | 'doc' | 'standards' | 'checklist' | 'help';
  readonly prompt: string;
}

function cleanupResourceName(value: string): string {
  return value
    .replace(/^(para|de|do|da|dos|das|um|uma|o|a)\s+/i, '')
    .trim();
}

export function detectResourceIntent(prompt: string): ResourceIntent | undefined {
